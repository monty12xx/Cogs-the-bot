    ...
    async def help(self, ctx, *commands : str):
        ...
        if my.permissions_in(channel).embed_links:
            ...
            if len(pages) == 1:
                ...
                page_no = 1
                about = "T3D- is a simple bot, funbot, musicbot and stuff like that\nto invite T3D just type %invite\nthis bot is owned by teddy and made by him, alan helped the most,and a few help from dangerous\nthanks for using T3D-V9"
                colour = ''.join([randchoice('0123456789ABCDEF') for x in range(6)])
                colour = int(colour, 16)
